...
class CsvData(models.Model):
    ...
    def __str__(self):
        return f"{self.banco} - {self.layout} - {self.cpf} - {self.nome} - {self.natureza_do_lancamento} - {self.tipo_de_lancamento} - {self.observacao} - {self.bruto_contrato} - {self.liquido_contrato} - {self.vlr_lancamento} - {self.percentual_lancamento} - {self.data_da_proposta} - {self.data_do_lancamento}"
